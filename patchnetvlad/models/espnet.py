...
class ESPNet(nn.Module):
    ...
    def forward(self, input):
        ...
        # print (output0.shape, " and ", output1_C.shape," ", output2_c.shape)
        comb_l2_l3 = self.up_l2(self.combine_l2_l3(torch.cat([output1_C, output2_c], 1))) #RUM

        # print (output0.shape, " and ", comb_l2_l3.shape)

        if comb_l2_l3.shape[2] < output0.shape[2]:
            output0 = output0[:,:,0:comb_l2_l3.shape[2], :]
        if comb_l2_l3.shape[2] > output0.shape[2]:
            comb_l2_l3 = comb_l2_l3[:,:,0:output0.shape[2], :]
        if comb_l2_l3.shape[3] > output0.shape[3]:
            comb_l2_l3 = comb_l2_l3[:,:,:,0:comb_l2_l3.shape[3]-1]
        if comb_l2_l3.shape[3] < output0.shape[3]:
            output0 = output0[:,:,:,0:output0.shape[3]-1]
